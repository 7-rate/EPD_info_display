...
#define Large 15 // For icon drawing
...
long StartTime = 0;
long SleepTimer = 0;
long Delta = 600; // ESP32 rtc speed compensation, prevents display at xx:59:yy and then xx:00:yy
                  // (one minute later) to save power
...
void addcloud(int x, int y, int scale, int linesize) {
    ...
    fillCircle(x + scale * 1.5, y - scale * 1.3, scale * 1.75,
               Black); // Right middle upper circle
    fillRect(x - scale * 3 - 1, y - scale, scale * 6, scale * 2 + 1,
             Black);                                                 // Upper and lower lines
    ...
    fillCircle(x + scale * 1.5, y - scale * 1.3, scale * 1.75 - linesize,
...
void addtstorm(int x, int y, int scale) {
    y = y + scale / 2;
    for (int i = 0; i < 5; i++) {
        drawLine(x - scale * 4 + scale * i * 1.5 + 0, y + scale * 1.5,
                 x - scale * 3.5 + scale * i * 1.5 + 0, y + scale, Black);
        if (scale != Small) {
            drawLine(x - scale * 4 + scale * i * 1.5 + 1, y + scale * 1.5,
                     x - scale * 3.5 + scale * i * 1.5 + 1, y + scale, Black);
            drawLine(x - scale * 4 + scale * i * 1.5 + 2, y + scale * 1.5,
                     x - scale * 3.5 + scale * i * 1.5 + 2, y + scale, Black);
        }
        drawLine(x - scale * 4 + scale * i * 1.5, y + scale * 1.5 + 0,
                 x - scale * 3 + scale * i * 1.5 + 0, y + scale * 1.5 + 0, Black);
        if (scale != Small) {
            drawLine(x - scale * 4 + scale * i * 1.5, y + scale * 1.5 + 1,
                     x - scale * 3 + scale * i * 1.5 + 0, y + scale * 1.5 + 1, Black);
            drawLine(x - scale * 4 + scale * i * 1.5, y + scale * 1.5 + 2,
                     x - scale * 3 + scale * i * 1.5 + 0, y + scale * 1.5 + 2, Black);
        }
        drawLine(x - scale * 3.5 + scale * i * 1.4 + 0, y + scale * 2.5,
                 x - scale * 3 + scale * i * 1.5 + 0, y + scale * 1.5, Black);
        if (scale != Small) {
            drawLine(x - scale * 3.5 + scale * i * 1.4 + 1, y + scale * 2.5,
                     x - scale * 3 + scale * i * 1.5 + 1, y + scale * 1.5, Black);
            drawLine(x - scale * 3.5 + scale * i * 1.4 + 2, y + scale * 2.5,
...
void DisplayForecastWeather(int x, int y, int index) {
    ...
    drawString(x + fwidth / 2, y + 30,
               String(ConvertUnixTime(WxForecast[index].Dt).substring(0, 5)), CENTER);
    // 気温
    drawString(x + fwidth / 2, y + 125, String(WxForecast[index].High, 0) + "°", CENTER);
}
...
void DrawGraph(int x_pos, int y_pos, int gwidth, int gheight, float Y1Min, float Y1Max,
               String title, float DataArray[], int readings, boolean auto_scale,
...
#define y_minor_axis 5 // 5 y-axis division markers
    ...
    if (auto_scale == true) {
        ...
        maxYscale =
            round(maxYscale + auto_scale_margin); // Auto scale the graph and round to the
                                                  // nearest value defined, default was Y1Max
        Y1Max = round(maxYscale + 0.5);
        if (minYscale != 0)
            minYscale =
                round(minYscale - auto_scale_margin); // Auto scale the graph and round to the
...
void DisplayForecastSection(int x, int y) {
    ...
    for (int i = 0; i < max_readings; i++) {
        DisplayForecastWeather(x, y, i);
    }

    // セルシウス温度/華氏温度変換
    for (int i = 0; i < max_readings; i++) {
        if (Units == "I") {
            pressure_readings[i] = WxForecast[i].Pressure * 0.02953;
            rain_readings[i] = WxForecast[i].Rainfall * 0.0393701;
            snow_readings[i] = WxForecast[i].Snowfall * 0.0393701;
        } else {
            pressure_readings[i] = WxForecast[i].Pressure;
            rain_readings[i] = WxForecast[i].Rainfall;
            snow_readings[i] = WxForecast[i].Snowfall;
        }

        temperature_readings[i] = WxForecast[i].Temperature;
        humidity_readings[i] = WxForecast[i].Humidity;
    }
    ...
    DrawGraph(gx + 0 * gap, gy, gwidth, gheight, 900, 1050,
              Units == "M" ? TXT_PRESSURE_HPA : TXT_PRESSURE_IN, pressure_readings, max_readings,
              autoscale_on, barchart_off);
    DrawGraph(gx + 1 * gap, gy, gwidth, gheight, 10, 30,
              Units == "M" ? TXT_TEMPERATURE_C : TXT_TEMPERATURE_F, temperature_readings,
              max_readings, autoscale_on, barchart_off);
...
static void connect_wifi() {
    int32_t primary_rssi = -1000;
    int32_t secondary_rssi = -1000;
    ...
    for (int i = 0; i < n; ++i) {
        const char *ssid = WiFi.SSID(i).c_str(); // ステルスSSIDは取得できない...
        ...
        if (strcmp(ssid, SSID) == 0) {
            primary_rssi = rssi;
        } else if (strcmp(ssid, SSID_sub) == 0) {
            secondary_rssi = rssi;
        }
    }

    if (primary_rssi >= secondary_rssi) {
        WiFi.begin(SSID, PASS);
    } else {
        WiFi.begin(SSID_sub, PASS_sub);
    }
...
boolean UpdateLocalTime() {
    ...
    Serial.println(&timeinfo,
                   "%a %b %d %Y   %H:%M:%S"); // Displays: Saturday, June 24 2017 14:05:49
    if (Units == "M") {
        sprintf(day_output, "%s, %02u %s %04u", weekday_D[timeinfo.tm_wday], timeinfo.tm_mday,
                month_M[timeinfo.tm_mon], (timeinfo.tm_year) + 1900);
        strftime(update_time, sizeof(update_time), "%H:%M:%S",
                 &timeinfo); // Creates: '@ 14:05:49'   and change from 30 to 8
                             // <<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
        sprintf(time_output, "%s", update_time);
    } else {
        strftime(day_output, sizeof(day_output), "%a %b-%d-%Y",
                 &timeinfo);                                         // Creates  'Sat May-31-2019'
...
boolean SetupTime() {
    configTime(gmtOffset_sec, daylightOffset_sec, ntpServer,
               "time.nist.gov"); //(gmtOffset_sec, daylightOffset_sec, ntpServer)
...
bool DecodeWeather(WiFiClient &json, String Type) {
    ...
    if (Type == "weather") {
        // All Serial.println statements are for diagnostic purposes and some are not required,
        // remove if not needed with //
        // WxConditions[0].lon         = root["coord"]["lon"].as<float>(); Serial.println(" Lon:
        // " + String(WxConditions[0].lon)); WxConditions[0].lat         =
        // root["coord"]["lat"].as<float>();              Serial.println(" Lat: " +
        // String(WxConditions[0].lat));
    ...
    if (Type == "forecast") {
        ...
        for (byte r = 0; r < max_readings; r++) {
            ...
            Serial.println("Dt: " + String(WxForecast[r].Dt));
            ...
            // WxForecast[r].Description       =
            // list[r]["weather"][0]["description"].as<char*>(); Serial.println("Desc: " +
            // String(WxForecast[r].Description)); WxForecast[r].Cloudcover        =
            // list[r]["clouds"]["all"].as<int>(); Serial.println("CCov: " +
            // String(WxForecast[r].Cloudcover)); // in % of cloud cover WxForecast[r].Windspeed
            // = list[r]["wind"]["speed"].as<float>(); Serial.println("WSpd: " +
            // String(WxForecast[r].Windspeed)); WxForecast[r].Winddir =
            // list[r]["wind"]["deg"].as<float>();               Serial.println("WDir: " +
            // String(WxForecast[r].Winddir));
...
void DisplayDisplayWindSection(int x, int y, float angle, float windspeed, int Cradius) {
    arrow(x, y, Cradius - 22, angle, 18,
          33); // Show wind direction on outer circle of width and length
    setFont(OpenSans8B);
    int dxo, dyo, dxi, dyi;
...
void DisplayAstronomySection(int x, int y) {
    ...
    drawString(x + 5, y + 50,
               ConvertUnixTime(WxConditions[0].Sunset).substring(0, 5) + " " + TXT_SUNSET, LEFT);
...
void DisplayTemperatureSection(int x, int y) {
    setFont(OpenSans18B);
    drawString(x - 30, y,
               String(WxConditions[0].Temperature, 1) + "°    " +
                   String(WxConditions[0].Humidity, 0) + "%",
               LEFT);
    setFont(OpenSans12B);
    drawString(x + 10, y + 35,
               String(WxConditions[0].High, 0) + "° | " + String(WxConditions[0].Low, 0) + "°",
               CENTER); // Show forecast high and Low
}
...
#define lineWidth 34
    ...
    if (WxForecast[0].Rainfall > 0) {
        String Wx_Description =
            "Rain:" + String(WxForecast[0].Rainfall, 1) + String((Units == "M" ? "mm" : "in"));
...
void DrawPressureAndTrend(int x, int y, float pressure, String slope) {
    drawString(x + 25, y - 10,
               String(pressure, (Units == "M" ? 0 : 1)) + (Units == "M" ? "hPa" : "in"), LEFT);
    if (slope == "+") {
...
void DisplayMoney(int x, int y) {
    setFont(OpenSans12B);
    drawString(x, y + 0, "USD: ", LEFT);
    drawString(x + 180, y + 0, String(usd_jpy, 2), RIGHT);

    drawString(x, y + 30, "BTC: ", LEFT);
    drawString(x + 180, y + 30, String(btc_jpy, 0), RIGHT);

    drawString(x, y + 60, "XRP: ", LEFT);
    drawString(x + 180, y + 60, String(xrp_jpy, 2), RIGHT);

    drawString(x, y + 90, "ETH: ", LEFT);
    drawString(x + 180, y + 90, String(eth_jpy, 0), RIGHT);
}
...
bool obtainWeatherData(WiFiClientSecure &client, const String &RequestType) {
    ...
    http.begin(client, server, 443,
               uri); // http.begin(uri,test_root_ca); //HTTPS example connection
...
void BeginSleep() {
    ...
    // CurrentHour >= SleepHourの場合は、WakeUpHourまでの時間分加算する
    if (CurrentHour >= SleepHour) {
        SleepTimer += (24 - CurrentHour + WakeupHour - 1) * 3600;
    }

    esp_sleep_enable_timer_wakeup(SleepTimer *
                                  1000000LL); // in Secs, 1000000LL converts to Secs as unit = 1uSec
...
void obtainExchangeData(WiFiClientSecure &client) {
    HTTPClient http;
    http.begin(client, exchange_server, 443, exchange_uri);
    int httpCode = http.GET();
    ...
    if (httpCode == HTTP_CODE_OK) {
        ...
        usd_jpy = doc["conversion_rates"]["JPY"];
        Serial.printf("USD/JPY: %f\n", usd_jpy);
    } else {
...
void obtainVirtualCoinData(WiFiClientSecure &client) {
    HTTPClient http;
    int httpCode;

    // BTC
    http.begin(client, crypto_server, 443, crypto_btc_uri);
    httpCode = http.GET();
    Serial.printf("httpCode: %d\n", httpCode);
    ...
    if (httpCode == HTTP_CODE_OK) {
        ...
        JsonDocument doc;
        deserializeJson(doc, payload);
        btc_jpy = doc[0]["current_price"];
        Serial.printf("BTC/JPY: %f\n", btc_jpy);
    } else {
        Serial.printf("HTTP request failed, error: %s\n", http.errorToString(httpCode).c_str());
    }
    http.end();
    ...
    // XRP
    http.begin(client, crypto_server, 443, crypto_xrp_uri);
    httpCode = http.GET();
    Serial.printf("httpCode: %d\n", httpCode);
    ...
    if (httpCode == HTTP_CODE_OK) {
        ...
        JsonDocument doc;
        deserializeJson(doc, payload);
        xrp_jpy = doc[0]["current_price"];
        Serial.printf("XRP/JPY: %f\n", xrp_jpy);
    } else {
        Serial.printf("HTTP request failed, error: %s\n", http.errorToString(httpCode).c_str());
    }
    http.end();
    ...
    // ETH
    http.begin(client, crypto_server, 443, crypto_eth_uri);
    httpCode = http.GET();
    Serial.printf("httpCode: %d\n", httpCode);
    ...
    if (httpCode == HTTP_CODE_OK) {
        ...
        JsonDocument doc;
        deserializeJson(doc, payload);
        eth_jpy = doc[0]["current_price"];
        Serial.printf("ETH/JPY: %f\n", eth_jpy);
    } else {
        Serial.printf("HTTP request failed, error: %s\n", http.errorToString(httpCode).c_str());
    }
    http.end();
...
void setup() {
    ...
    if (StartWiFi() == WL_CONNECTED && SetupTime() == true) {
        // WifiClient初期化(httpsを使うため、Secureのほう)
        ...
        while ((RxWeather == false || RxForecast == false) &&
               Attempts <= 2) { // Try up-to 2 time for Weather and Forecast data
            if (RxWeather == false)
        ...
        /// 為替情報取得
        Serial.println("---- GET EXCHANGE ----");
        obtainExchangeData(client);
        client.stop();

        /// 仮想通貨情報取得
        Serial.println("---- GET VIRTUAL COIN ----");
        obtainVirtualCoinData(client);
        client.stop();
        ...
        if (RxWeather && RxForecast) { // Only if received both Weather or Forecast proceed
            ...
            Display();                 // Display the weather data
