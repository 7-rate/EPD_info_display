...
void write_string_bg_color(const GFXfont *font, const char *string, int *cursor_x,
                  int *cursor_y, uint8_t *framebuffer, int bg_color)
{
    char *token, *newstring, *tofree;
    if (string == NULL) {
        ESP_LOGE("font.c", "cannot draw a NULL string!");
    ...
    while ((token = strsep(&newstring, "\n")) != NULL) {
        *cursor_x = line_start;
        ...
        write_mode(font, token, cursor_x, cursor_y, framebuffer, BLACK_ON_WHITE, &props);
        *cursor_y += font->advance_y;
    }
